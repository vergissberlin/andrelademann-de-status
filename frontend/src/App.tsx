...
const SiteList: FC = () => {
	const { isLoading, error, data } = useQuery(['sites'], () => client.site.List(), {
	...
	return (
		<>
			<div className="sm:flex sm:items-center dark:bg-gray-900 dark:text-gray-100">
				<div className="sm:flex-auto">
					<h1 className="text-xl font-semibold text-gray-900 dark:text-gray-200">Monitored Websites</h1>
...
const StatusBadge: FC<{ status: monitor.SiteStatus | undefined }> = ({ status }) => {
	const [bg, text, label] = {
	...
	return (
		<span className={`inline-flex items-center rounded-md px-2.5 py-0.5 text-sm font-medium uppercase ${bg} ${text}`}>
			{label}
		</span>
	)
}
